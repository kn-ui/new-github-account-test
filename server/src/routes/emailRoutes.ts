...
router.post('/announcement', async (req, res) => {
  console.log('--- Announcement Email Route Hit ---');
...
const { title, body, targetAudience, courseId, recipientUserId } = announcement;
  ...
  try {
    ...
    if (targetAudience === 'ALL_STUDENTS') {
      ...
      console.log(`Found ${studentEmails.length} students for ALL_STUDENTS.`);
    ...
    if (studentEmails.length > 0) {
      ...
      return res.status(200).json({ success: true, message: 'Announcement email sent successfully!' });
    } else {
