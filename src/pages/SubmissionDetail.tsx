...
export default function SubmissionDetail() {
  ...
  const [submission, setSubmission] = useState<HygraphSubmission | null>(null);
  const [assignment, setAssignment] = useState<HygraphAssignment | null>(null);
  const [course, setCourse] = useState<HygraphCourse | null>(null);
  const [studentName, setStudentName] = useState<string>('');
  const [gradeDialog, setGradeDialog] = useState(false);
  const [grade, setGrade] = useState<number>(0);
  const [feedback, setFeedback] = useState<string>('');
  const [editRequest, setEditRequest] = useState<HygraphEditRequest | null>(null);
